(function(){
  ...
  function bindInputEvents(){ 
    ...
    input.addEventListener('input', ()=>{ const q=input.value.trim(); if(q.length<2){ if(dropdown) dropdown.style.display='none'; return; } createDropdown(); dropdown.style.display='block'; syncDropdownPosition(); clearTimeout(timer); timer=setTimeout(()=>{ fetchResults(q).then(d=>renderResults(d,q)); },250); }); 
    input.addEventListener('keydown', e=>{ const term=input.value.trim(); if(e.key==='Enter'&&term.length>0){ window.location.href = SEARCH_PAGE + encodeURIComponent(term); } if(!dropdown||dropdown.style.display==='none') return; const focusables=dropdown.querySelectorAll('.hqs-item, .hqs-card a, .hqs-viewall'); if(!focusables.length) return; const arr=[...focusables]; const idx=arr.indexOf(document.activeElement); if(e.key==='ArrowDown'){ e.preventDefault(); (arr[Math.min(idx+1,arr.length-1)]||arr[0]).focus(); } else if(e.key==='ArrowUp'){ e.preventDefault(); (arr[Math.max(idx-1,0)]||arr[0]).focus(); } else if(e.key==='Escape'){ dropdown.style.display='none'; input.focus(); } }); 
    document.addEventListener('click', ev=>{ if(!dropdown) return; if(!dropdown.contains(ev.target)&&ev.target!==input){ dropdown.style.display='none'; } }); 
  }
